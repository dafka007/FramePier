#define AppName "FramePier"
#define AppVersion "0.2.2"
#define Publisher "FramePier"
#define AppExeName "FramePierHelper.exe"
...
DefaultDirName={localappdata}\Programs\FramePier
DefaultGroupName=FramePier
UsePreviousGroup=no
...
OutputDir=..\dist
OutputBaseFilename=FramePier-Setup-{#AppVersion}
Compression=lzma2/ultra64
SolidCompression=yes
WizardStyle=modern
SetupIconFile=..\assets\FramePier.ico
UninstallDisplayIcon={app}\FramePier.ico
VersionInfoVersion={#AppVersion}.0
VersionInfoCompany={#Publisher}
VersionInfoDescription=FramePier Installer
...
[Files]
Source: "..\dist\helper\FramePierHelper.exe"; DestDir: "{app}"; Flags: ignoreversion restartreplace uninsrestartdelete
Source: "..\dist\helper\FramePierHelper.exe"; DestName: "FramePierLifecycle.exe"; Flags: dontcopy
...
Source: "..\assets\FramePier.ico"; DestDir: "{app}"; Flags: ignoreversion
...
Type: files; Name: "{localappdata}\VidDock\.viddock-upgraded"

[InstallDelete]
Type: files; Name: "{app}\VidDockHelper.exe"
Type: files; Name: "{app}\VidDockLifecycle.exe"
Type: files; Name: "{app}\VidDock.ico"

[Icons]
Name: "{group}\FramePier Extension Folder"; Filename: "{app}\Extension"
Name: "{group}\Repair Browser Integration"; Filename: "{app}\FramePierHelper.exe"; Parameters: "--repair-ui"; WorkingDir: "{app}"
Name: "{group}\FramePier README"; Filename: "{app}\README.md"
Name: "{group}\Uninstall FramePier"; Filename: "{uninstallexe}"
...
[Run]
Filename: "{app}\FramePierHelper.exe"; Parameters: "--open-browser-setup"; Description: "Open browser extension setup"; Flags: postinstall nowait skipifsilent unchecked
...
procedure RemoveOldStartMenuGroup;
var
  OldGroup: String;
begin
  OldGroup := ExpandConstant('{userprograms}\VidDock');
...
procedure WriteUpgradeMarker;
var
  Marker: String;
begin
  Marker := ExpandConstant('{localappdata}\VidDock\.viddock-upgraded');
  DirExists(ExpandConstant('{localappdata}\VidDock'));
  SaveStringToFile(Marker, 'upgraded', True);
  Log('VidDock upgrade confirmed; migration marker written to ' + Marker);
end;
...
begin
  ...
  Result := Exec(ExpandConstant('{tmp}\FramePierLifecycle.exe'), Parameters, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
end;
...
begin
  Result := '';
  Target := ExpandConstant('{app}\FramePierHelper.exe');
  if not FileExists(Target) then
  begin
    if FileExists(ExpandConstant('{app}\VidDockHelper.exe')) then
    begin
      WasVidDockUpgrade := True;
      Log('VidDock upgrade detected; migration marker will be written after successful install.');
    end;
    Exit;
  end;
  ExtractTemporaryFile('FramePierLifecycle.exe');
  StatusCode := 0;
  if not RunLifecycle('--lifecycle-status', Target, StatusCode) then
  begin
    Result := 'FramePier could not inspect the existing helper process.';
    Exit;
  end;
  ...
  begin
    Result := 'FramePier could not validate the existing helper process.';
    Exit;
  end;
  if (StatusCode = 11) or (StatusCode = 12) then
  begin
    if StatusCode = 11 then
      PromptText := 'FramePier is currently downloading or merging a file. Continuing will safely cancel FramePier operations and remove their private temporary fragments before upgrading. Finished downloads will not be removed.'
    else
      PromptText := 'An older FramePier helper is running. Continuing will close it before upgrading. Any operation using that helper will be cancelled safely.';
    if (not WizardSilent) and (MsgBox(PromptText + #13#10 + #13#10 + 'Continue with the upgrade?', mbConfirmation, MB_YESNO) <> IDYES) then
    begin
      Result := 'Close or finish the current FramePier operation, then run Setup again.';
      Exit;
    end;
  end;
  SaveIntegration;
  ...
  begin
    ...
    Result := 'FramePierHelper.exe could not be closed safely. Setup has restored browser integration and made no file changes.';
    Exit;
  end;
  Log('FramePier helper processes stopped; existing Native Messaging registration was preserved.');
end;
...
begin
  ...
  BrowserPage := CreateInputOptionPage(wpSelectDir, 'Browser Integration',
    ...
    'FramePier will register its restricted Native Messaging host for selected installed browsers. Browser security still requires you to approve the local extension.', False, False);
...
begin
  if CurPageID = wpFinished then
    WizardForm.FinishedLabel.Caption :=
      'FramePier installed successfully.' + #13#10 + #13#10 +
      ...
      'FramePier requests a supported extension reload when an active older build detects this installed package. If the browser keeps stale unpacked code, completely exit and reopen it; click Reload only as a final fallback.' + #13#10 + #13#10 +
...
begin
  ...
  begin
    if WasVidDockUpgrade then
    begin
      WriteUpgradeMarker;
    end;
    if FileExists(ExpandConstant('{app}\FramePierHelper.exe')) then
    begin
      if (not Exec(ExpandConstant('{app}\FramePierHelper.exe'), '--repair', '', SW_HIDE, ewWaitUntilTerminated, ResultCode)) or (ResultCode <> 0) then
        RaiseException('FramePier post-install Native Messaging validation failed. Run Repair Browser Integration or reinstall.');
    end;
    RemoveOldStartMenuGroup;
    LifecycleQuiesced := False;
    InstallCompleted := True;
    // Regression check: fresh install must NOT have the marker file
    if not WasVidDockUpgrade and FileExists(ExpandConstant('{localappdata}\VidDock\.viddock-upgraded')) then
      RaiseException('Regression: fresh FramePier install must not create .viddock-upgraded marker');
  end;
end;
...
begin
  ...
  begin
    if FileExists(ExpandConstant('{tmp}\FramePierLifecycle.exe')) then
...
begin
  ...
  if (StatusCode <> 0) and (StatusCode <> 10) and (StatusCode <> 11) and (StatusCode <> 12) then
  begin
    MsgBox('FramePier could not verify its running processes. Uninstall was cancelled without changing browser integration.', mbError, MB_OK);
    Exit;
  end;
  if (StatusCode = 11) or (StatusCode = 12) then
  begin
    PromptText := 'FramePier is currently active. Uninstalling will safely cancel its operations and remove private temporary fragments. Finished downloads will not be removed.';
  ...
  begin
    ...
    RestoreSavedIntegration;
    MsgBox('FramePierHelper.exe could not be closed safely. Uninstall was cancelled and browser integration was restored.', mbError, MB_OK);
    Exit;
  end;
  Result := True;
end;
